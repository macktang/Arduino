...
//        Serial3 << "w axis" << axis << ".motor.config.current_lim " << 11.0f << '\n';
        ...
        while( (loop_current_time - loop_start_time) < 12000 ){
          ...
          if ( myPos >= 5606 && myPos <= 7426 ){
            myError = (myPos-6550)*0.0003834951969714103074295218974*.18;
            mySpeedDampen = abs(myPos - prevPos)*0.0003834951969714103074295218974 * 6.0;
//            Serial.println( (myPos-6443)*0.0003834951969714103074295218974*.18 ,7);
//            Serial.println(myError,7);
            myCurrent = -myError * 360. / 8.27 + mySpeedDampen*sgn(myPos-6550);
            if (myCurrent > 3.0){
              myCurrent = 3.0;
            }
            else if (myCurrent < -3.0){
              myCurrent = -3.0;
            }
...
//            Serial.println(myError,8);
            ...
            Serial3 << "w axis" << my_axis << ".controller.current_setpoint " << 0.0f << '\n';
//            Serial.println(0.0);
          }
