...
void Odrivesetup() {
  ...
  for (int axis = 0; axis < 2; ++axis) {
    odrive_serial << "w axis" << axis << ".controller.config.vel_limit " << 101000.0f << '\n';
...
void Odriveloop() {
  ...
  if (Serial.available()) {
    ...
    if (c == 's') {
      ...
      for (float ph = 0.0f; ph < 10*6.28318530718f; ph += 0.02f) {
        float pos_m0 = 2000.0f * cos(ph);
        float pos_m1 = 2000.0f * sin(ph);
...
//        odrive_serial << "w axis" << axis << ".motor.config.current_lim " << 11.0f << '\n';
        ...
        odrive_serial << "w axis" << my_axis << ".controller.config.control_mode " << control_mode << '\n';
...
//        odrv0.axis0.controller.current_setpoint = 0.4
        odrive_serial << "w axis" << my_axis << ".controller.current_setpoint " << 0.1f << '\n';
